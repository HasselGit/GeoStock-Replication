...
### Módulo: Control de Accesos y Usuarios (Actualizado)
...
### Módulo: Tambores y Ubicación Física (Actualizado)
...
### Módulo: Compras y Sincronización de Padrón ARCA/Finnegans (Actualizado)
...
*   **Sincronizador de Padrón**: Servicio automático y a demanda para sincronizar el padrón de productores de Finnegans (1487 registros) y validar CUITs de apicultores (`GET /compra/padron/estado`, `POST /compra/padron/sincronizar`).
...
*   **`EnvaseCera`**: Registro de envases de cera procesados en depósitos (código, tipo `PALLET`/`BOLSON`, pesoBruto, tara, pesoNeto, estado `EN_DEPOSITO`/`EXPORTADO`, fecha y observaciones). Soporta actualización explícita (`PATCH /almacenes/envases-cera/:id`).

### Módulo: Logística, Reservas de Stock y Remitos
...
*   **`ReservaStock`**: Control de apartado de existencias previas al despacho (`referenciaId`, `warehouseId`, `cantidad`, `estado` `ACTIVA`/`CONSUMIDA`/`CANCELADA`).
...
## 2. Contratos de la API y Flujos de Integración (Nuevos Endpoints 3 Agosto)

### A. Sincronización de Padrón ARCA/Finnegans
*   `GET /api/compra/padron/estado`: Retorna el estado del job automático diario de sincronización del padrón de apicultores con Finnegans.
*   `POST /api/compra/padron/sincronizar`: Ejecuta la sincronización inmediata bajo demanda.

### B. Consulta Directa SENASA
*   `POST /api/senasa/consultar`: `{ senasaCode, barrelId }` $\rightarrow$ Consulta directamente el webservice de SENASA para validar trazabilidad.

### C. Gestión de Tambores y Cera
*   `DELETE /api/barrels/:id`: Elimina un registro de tambor.
*   `PATCH /api/almacenes/envases-cera/:id`: Modifica datos de un envase de cera existente.
...
3.  **Almacenes**: `/api/almacenes`, `/api/almacenes/referencias` (GET, POST, PATCH), `/api/almacenes/existencias` (GET), `/api/almacenes/referencias/:id/movimientos` (GET), `/api/almacenes/movimientos` (POST), `/api/almacenes/envases-cera` (GET, POST, PATCH, DELETE), `/api/almacenes/envases-cera/:id/exportar` (POST).
...
5.  **Laboratorio y SENASA**: `/api/senasa/consultar` (POST), `/api/laboratorio/ordenes`, `/api/laboratorio/iniciar` (POST), `/api/laboratorio/muestras/:id` (PATCH), `/api/laboratorio/ordenes/:id/validar-muestra` (POST), `/api/laboratorio/canastos` (GET, POST), `/api/laboratorio/canastos/:id` (GET, PATCH, DELETE, POST `/archivar`).
